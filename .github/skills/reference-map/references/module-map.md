...
| `:processor` | `:annotations` | `codegen/`, `extensions/`, `factory/`, `logger/`, `model/` | KSP processor (`Provider` -> `Processor`) that reads `@Entity`, `@ColumnInfo`, `@Embedded` and generates schema object classes via KotlinPoet | `https://anitrend.github.io/support-query-builder/processor/index.html` |
...
- `:processor` — `Provider.kt` is the `SymbolProcessorProvider` entry point and `Processor.kt` is the `SymbolProcessor` implementation; `model/` contains `Candidate`, `ColumnItem`, `EmbedItem`, `FieldItem`, `TableItem`; `factory/ClassFactory.kt` drives KotlinPoet code generation; `logger/` provides a logging abstraction.

## Consumer Notes

- Consumers add `:core` for the query builder, `:core:ext` for Room `@RawQuery` integration, and `:annotations` + `:processor` (via `ksp`) for compile-time schema generation.
